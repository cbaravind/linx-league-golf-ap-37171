import { View, Text, StyleSheet, FlatList, Share, Linking, ScrollView, Keyboard, TouchableWithoutFeedback } from 'react-native'
...
export default function SendReferral({ route }) {
  ...
  const searchFriends = (searchText) => {
    ...
    if (searchText.length && contactsArray.length) {
      ...
      const filtered = contactsArray.filter(e => e.givenName.includes(searchText))
      ...
      <View style={{ backgroundColor: colors.background,flex:1, paddingTop: 30 }} >
        <KeyboardAvoidingView style={{ flex: 1,}} behavior={'height'} >
          ...
          <>
              <ScrollView style={{ flex: 1,}}>
                ...
                {contactsArray.length ?
                  <>
                    <FlatList
                      data={contactsArray}
                      contentContainerStyle={{ backgroundColor: colors.white, padding: 20, marginTop: 20, marginBottom: 12, paddingBottom: 30 }}
              ...
              <Button
                marginY={'3'} marginX={'7'} shadow={5} bg='#7D9E49'>{"Send"}</Button>
          </>
